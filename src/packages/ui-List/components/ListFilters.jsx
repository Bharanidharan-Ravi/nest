            ...
            <button
              ...
              onClick={() => {
                if (isOpen) {
                  ...
                  const rect =
                    triggerRefs.current[filter.key]?.getBoundingClientRect();
                  if (rect) {
                    setDropdownPosition({
                      top: rect.bottom + 4,
                ...
                >
                  ...
                  <div className="overflow-y-auto flex-1 py-1 custom-scrollbar">
                    {filteredOptions.map((opt, index) => {
                      ...
                      const hasRealSelection = selectedValues.some(
                        (v) => v !== "",
                      );
                      ...
                      const handleSelect = () => {
                        if (isAll) {
                          // Clear every selected value for this filter
                          updateQuery(filter.key, [], true);
                        ...
                        const currentSelected = selectedValues.filter((v) => {
                          return v !== "" && v !== null && v !== undefined;
                        });
                        if (isMultiSelect) {
                          const value = String(opt.value);

                          let newSelection = currentSelected.filter(
                            (v) => v !== "",
                          );
                          ...
                          updateQuery(filter.key, newSelection, true);

                          // Keep dropdown open for multi-select
                          setHighlightedIndex(0);
                        } else {
                          updateQuery(filter.key, [String(opt.value)], false);

                          // Close single-select dropdown
                          setOpenDropdownKey(null);
                        }
                      };
