...
const MILD_COLORS = [
    ...
    "#A78BFA", "#2DD4BF", "#FB7185", "#A3E635", "#FACC15",
    "#60A5FA", "#4ADE80", "#F87171", "#818CF8", "#C084FC"
];
    ...
    const xaxis = useMemo(() => {
        ...
        const keys = [...new Set(
            data.map(item => {
                ...
                return graphConfig.isDateAxis ? (rawDate ? rawDate.split("T")[0] : null) : rawDate;
            }).filter(Boolean)
        )].sort();
console.log("keys :", keys , data);
    ...
    const chartData = useMemo(() => {
        ...
        data.forEach((item) => {
            const rawDate = getSafeValue(item, graphConfig.graphXAxisKey);
            const xKey = graphConfig.isDateAxis ? (rawDate ? rawDate.split("T")[0]:null) : rawDate;
           
            if (!grouped[xKey]) return;

            const val = parseValue(getSafeValue(item, graphConfig.graphValueKey));
            if (val > 0) {
                const uniqueId = getSafeValue(item, graphConfig.graphGroupIdKey) || item.id || "unknown";
                const finalColor = getSafeValue(item, graphConfig.graphColorKey) || getHashColor(uniqueId);
                
                if (!grouped[xKey][uniqueId]) {
                    grouped[xKey][uniqueId] = {
                        rawItem: item, 
                        value: 0,
                        label: getSafeValue(item, graphConfig.graphLabelKey) || "Unknown",
                        color: finalColor,
                        recordCount: 0,
                        terminalCount: 0,
                    };
        ...
        Object.keys(grouped).forEach(xKey => {
            ...
            finalGrouped[xKey].forEach(seg => {
                seg.isTerminal = seg.terminalCount > 0 && seg.terminalCount === seg.recordCount;
                seg.display = graphConfig.valueFormatter ? graphConfig.valueFormatter(seg.value) : seg.value;
            });
        });
            ...
            {containerW > 0 && (
                <div ref={scrollRef} className="overflow-auto custom-scrollbar relative" style={{ maxHeight: `${viewportMaxH}px` }}>
                    <div style={{ minWidth: `${totalW}px`, height: `${totalH}px` }} className="relative bg-white">
                        ...
                        <div className="sticky left-0 top-0 z-20 bg-white/95 backdrop-blur-sm border-r border-gray-100" style={{ width: yAxisW, height: innerH + padTop, float: 'left' }}>
                            {ticks.map(val => (
                                <div key={`y-${val}`} className="absolute w-full text-right pr-3 text-[11px] font-bold text-gray-400" style={{ top: getYPos(val) - 7 }}>
                                    {val}h
                                </div>
                            ))}
                        </div>
                        ...
                        <svg className="absolute top-0" style={{ left: yAxisW }} width={innerW} height={innerH + padTop}>
                            {ticks.map(val => (
                                <line key={`grid-${val}`} x1={0} x2={innerW} y1={getYPos(val)} y2={getYPos(val)} stroke="#f1f5f9" strokeWidth="1.5" strokeDasharray="4 4" />
                            ))}
                            ...
                            {xaxis.map((col, i) => {
                                ...
                                const dayTotalValue = (chartData[col.key] || []).reduce((sum, item) => sum + item.value, 0);
                                const dayTotalDisplay = graphConfig.valueFormatter ? graphConfig.valueFormatter(dayTotalValue) : `${dayTotalValue}h`;
                                ...
                                return (
                                    <g key={col.key}>
                                        {(chartData[col.key] || []).map((item, idx) => {
                                            ...
                                            return (
                                                <g key={idx} style={{ animation: `smoothDrop 0.6s cubic-bezier(0.25, 1, 0.5, 1) ${idx * 0.1}s forwards`, opacity: 0 }}>
                                                    <g 
                                                       ...
                                                       onClick={() => config.onItemClick && config.onItemClick(item.rawItem)}
                                                    >
                                                        <path d={`M ${x} ${y} L ${x} ${y + h} A ${rx} ${ry} 0 0 0 ${x + barW} ${y + h} L ${x + barW} ${y} Z`} fill={item.color} stroke="#ffffff" strokeWidth="1.5" strokeLinejoin="round" />
                                                        <path d={`M ${x} ${y} L ${x} ${y + h} A ${rx} ${ry} 0 0 0 ${x + barW} ${y + h} L ${x + barW} ${y} Z`} fill="url(#cylinderShine)" />
                                                        <ellipse cx={x + rx} cy={y} rx={rx} ry={ry} fill={item.color} stroke="#ffffff" strokeWidth="1.5" />
                                                        <ellipse cx={x + rx} cy={y} rx={rx} ry={ry} fill="url(#topShine)" />
                                                        
                                                        {item.isTerminal && (
                                                            <g className="pointer-events-none">
                                                                <path d={`M ${x} ${y} L ${x} ${y + h} A ${rx} ${ry} 0 0 0 ${x + barW} ${y + h} L ${x + barW} ${y} Z`} fill="url(#diagonalStripes)" />
                                                                <ellipse cx={x + rx} cy={y} rx={rx} ry={ry} fill="url(#diagonalStripes)" />
                                                                <line
                                                                    x1={x + barW + 13}
                                                                    y1={y + h / 2}
                                                                    x2={x + barW + 3}
                                                                ...
                                                                />
                                                                <circle cx={x + barW + 18} cy={y + h / 2} r="5.5" fill="#1e293b" stroke="#ffffff" strokeWidth="1" />
                                                                <text x={x + barW + 18} y={y + h / 2 + 2.5} fill="#ffffff" fontSize="7" fontWeight="bold" textAnchor="middle">
                                                                    C
                                                                </text>
                                                            </g>
                                                        )}
                        ...
                        <div className="sticky bottom-0 z-30 flex bg-white border-t border-gray-100" style={{ width: '100%', height: xAxisH, clear: 'both' }}>
                            <div className="sticky left-0 z-40 bg-white border-r border-gray-100" style={{ width: yAxisW, height: '100%' }}></div>
                            <div className="relative" style={{ width: innerW }}>
                                {xaxis.map((col, i) => {
                                    const segmentW = innerW / xaxis.length;
                                    return (
                                        <div key={col.key} className="absolute text-[11px] font-bold text-gray-500 whitespace-nowrap transform -translate-x-1/2" style={{ left: (i * segmentW) + segmentW/2, top: '12px' }}>
                                            {col.label}
                                        </div>
                                    );
                                })}
    ...
    const grouped = useMemo(() => {
        return data.reduce((acc, item) => {
            ...
            if (!acc[key]) acc[key] = { 
                value: 0, 
                rawItem: item, 
                color: getSafeValue(item, graphConfig.graphColorKey) || getHashColor(key) 
            };
            acc[key].value += val;
            return acc;
        }, {});
    ...
    return (
        <svg viewBox={`0 0 ${SIZE} ${SIZE}`} className="w-full h-auto max-w-[400px]">
            {Object.entries(grouped).map(([label, item], idx) => {
                ...
                return (
                    <path key={idx} d={pathData} fill={item.color} stroke="#fff" strokeWidth="3"
                        onMouseEnter={(e) => setTooltip({ 
                            label, 
                            display: graphConfig.valueFormatter ? graphConfig.valueFormatter(item.value) : item.value, 
                            x: e.clientX, 
                            y: e.clientY, 
                            color: item.color 
                        })}
...
export function ListGraphView() {
    const { data, config, filters } = useList();
...
console.log("graph data :", data);
    ...
    const graphConfig = useMemo(() => {
        ...
        return typeof baseConfig === "function" ? baseConfig(filters) : baseConfig;
    }, [config, filters]);

    if (!data || data.length === 0) return null;

    // 🚀 Dynamic Tooltip Extraction based on Dashboard instructions
    const tooltipSecondaryVal = tooltip && graphConfig.tooltipSecondaryLabelKey 
    ...
    return (
        <div className="w-full h-full flex justify-center items-start p-6 bg-transparent">
            {graphConfig.graphType === "pie" ? (
                <PieChartDesign data={data} graphConfig={graphConfig} setTooltip={setTooltip} config={config} />
            ) : (
                <StackedBarDesign data={data} graphConfig={graphConfig} setTooltip={setTooltip} config={config} />
            )}
            ...
            {tooltip && (
                ...
                >
                    <div className="flex items-center gap-2 mb-1">
                        <div className="w-2.5 h-2.5 rounded-full" style={{ backgroundColor: tooltip.color }}></div>
