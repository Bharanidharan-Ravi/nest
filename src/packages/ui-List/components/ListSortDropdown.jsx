...
export function ListSortDropdown() {
  const { config, sortField, setSortField, sortOrder, setSortOrder } =
    useList();
  ...
  const activeFieldConfig =
    config.sortFields.find((f) => f.key === sortField) || config.sortFields[0];
  ...
  const orderLabel =
    activeOrders.find((o) => o.key === sortOrder)?.label ||
    activeOrders[0]?.label;
