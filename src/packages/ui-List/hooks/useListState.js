...
//         if (filterConf.customParams) {
//           Object.assign(result, filterConf.customParams);
//         }
//       }
//     });
...
export function useListState(config, rawData = [], userRole = null) {
  ...
  const defaultTab = config.enableTabs !== false ? config.tabConfig?.[0]?.key : null;

  const initialView = (() => {
    if (isUrlSyncEnabled && searchParams.has(`${prefix}view`)) {
      return searchParams.get(`${prefix}view`);
    }
    if (isUrlSyncEnabled && hasMultipleViews) {
      try {
        const moduleCache = JSON.parse(
          sessionStorage.getItem(`wgnest_cache_${currentModule}`) || "{}",
        );
        if (moduleCache.view) return moduleCache.view;
      } catch (e) {
        console.error("Cache read failed", e);
      }
    }
  ...
  const VIEW_CACHE_KEY = hasMultipleViews
    ? `wgnest_cache_${currentModule}_${initialView}`
    : `wgnest_cache_${currentModule}`;
  ...
  const getInitialState = (urlKey, stateKey, fallback) => {
    ...
    if (!urlHasFilters) {
      try {
        const cacheStr = sessionStorage.getItem(VIEW_CACHE_KEY);
        if (cacheStr) {
          const cachedData = JSON.parse(cacheStr);
          if (cachedData[stateKey] !== undefined) return cachedData[stateKey];
        }
      } catch (e) {
        console.error("Cache read failed", e);
      }
    ...
    (newView) => {
      ...
      if (hasMultipleViews) {
        ...
        sessionStorage.setItem(
          `wgnest_cache_${currentModule}_${view}`,
          JSON.stringify({
            query,
            sortField,
            sortOrder,
        ...
        // Defaults for incoming view
        let nextQuery = buildDefaultString(defaultTab);
        let nextSortField = config.defaultSort?.field || "updatedAt";
        let nextSortOrder = config.defaultSort?.order || "desc";
        let nextFilters = {};

        try {
          const cacheStr = sessionStorage.getItem(
            `wgnest_cache_${currentModule}_${newView}`,
          );

          if (cacheStr) {
            const cached = JSON.parse(cacheStr);

            if (cached.query !== undefined) {
              nextQuery = cached.query;
            }

            if (cached.sortField !== undefined) {
              nextSortField = cached.sortField;
            }

            if (cached.sortOrder !== undefined) {
              nextSortOrder = cached.sortOrder;
            }

            if (cached.filters !== undefined) {
              nextFilters = cached.filters;
            }
          }
        } catch (e) {
          console.error("Cache read failed", e);
        ...
        // Let React commit the new state before enabling rendering
  ...
  return {
    ...
    setView: changeView,
    data: visibleData,
    total: switchingView ? 0 : processed.length,
    hasMore: !switchingView && visibleCount < processed.length,
    loadMore,
    dataUpdatedAt,
    selectedOptions,
    setSelectedOptions,
