...
const BOOKING_COLORS = {
  meeting: {
    bg: "bg-[#e2ddfc]", borderL: "border-l-[#6e58d1]", text: "text-[#251f47]",
    textDim: "text-[#5a4a9a]", dot: "bg-[#6e58d1]", badge: "bg-[#6e58d1] text-white"
  },
  availability: {
    bg: "bg-[#55db8d]/20", borderL: "border-l-[#21b35b]", text: "text-[#103621]",
    textDim: "text-[#2d7a4f]", dot: "bg-[#21b35b]", badge: "bg-[#21b35b] text-white"
  },
  interview: { bg: "bg-[#d6aef8]/20", borderL: "border-l-[#6e468d]", text: "text-[#332240]", textDim: "text-[#6b4c7a]", dot: "bg-[#6e468d]", badge: "bg-[#6e468d] text-white" },
  demo: { bg: "bg-[#f5ce8c]/20", borderL: "border-l-[#7a602d]", text: "text-[#332917]", textDim: "text-[#7a5f3a]", dot: "bg-[#7a602d]", badge: "bg-[#7a602d] text-white" },
  discussion: { bg: "bg-[#81e1f5]/20", borderL: "border-l-[#246977]", text: "text-[#112d33]", textDim: "text-[#3a6b77]", dot: "bg-[#246977]", badge: "bg-[#246977] text-white" },
  supportCall: { bg: "bg-[#f7a6b8]/20", borderL: "border-l-[#7a3443]", text: "text-[#361a20]", textDim: "text-[#7a4250]", dot: "bg-[#7a3443]", badge: "bg-[#7a3443] text-white" },
};
...
function TimelineBlock({ event, isAvailable, onClick }) {
  const type = isAvailable
    ? "availability"
    : event.booking_type?.toLowerCase() || "meeting";
  ...
  const attendanceCount =
    internalParticipants.length + clientParticipants.length;

  // Convert "010101" → day names
  ...
  return (
    <button
      onClick={() =>
        onClick?.(isAvailable ? "availability" : "meeting", event)
      }
      className={`w-full flex items-center justify-between rounded-[4px] border-l-[3px] border-y border-r border-gray-200 px-4 py-3 text-left transition-all hover:opacity-90 ${colors.bg} ${colors.borderL} mb-2`}
    >
      <div>
        {/* Title */}
        <span className={`block text-[13px] font-semibold mb-1 ${colors.text}`}>
        ...
        <div className={`flex items-center gap-1.5 text-[11px] ${colors.textDim}`}>
          ...
          {recurrenceType && (
            <span>
              {recurrenceType}
              {/* {recurrenceType === "Weekly" && formattedDays
                ? ` (${formattedDays})`
                : ""} */}
            </span>
          )}
      ...
      {/* Badge */}
      <span
        className={`text-[11px] font-medium px-2.5 py-0.5 rounded ${colors.badge} capitalize`}
      >
        {isAvailable ? "Available" : type}
      </span>
    </button>
...
function MeetingCard({ event, isAvailable, onClick, onEdit }) {
  ...
  return (
    <div
      ...
      className="relative cursor-pointer rounded-xl border border-gray-200 bg-white p-4 flex flex-col gap-3 hover:bg-gray-50 transition-colors"
    >
      <button
        onClick={(e) => {
          e.stopPropagation();
        ...
        className="absolute top-3 right-3 p-1.5 rounded-md hover:bg-gray-100 text-gray-500 hover:text-gray-700 transition"
      >
        <FaEdit size={14} />
      </button>
      ...
      <div className="flex items-center justify-between pr-6">
        <h3 className="text-sm font-semibold text-gray-900 truncate">{event.title || (isAvailable ? "Availability Block" : "Untitled")}</h3>
        <span className={`text-[9px] font-bold uppercase tracking-wide px-1.5 py-0.5 rounded ${statusColor}`}>
          {event.status || 'Active'}
        </span>
      </div>
...
function CalendarSidebar({ meetings, availability, selected, setSelected }) {
  ...
  return (
    <aside className="w-[280px] shrink-0 bg-gray-100 border-r border-gray-300 flex flex-col h-full overflow-y-auto text-gray-700">
      <div className="px-5 py-5 flex items-center gap-2 text-gray-900 border-b border-gray-300">
        <FaCalendarAlt className="text-[#6e58d1] text-lg" />
        <h2 className="text-[15px] font-bold tracking-wide">Calander</h2>
      </div>


      <div className="px-5 pt-5 pb-4 border-b border-gray-300">
        <div className="flex items-center justify-between mb-4">
          <button onClick={() => setViewMonth(v => v.subtract(1, "month"))} className="p-2 rounded-md bg-white border border-gray-200 hover:bg-gray-50 text-gray-600 transition-colors">
            <FaChevronLeft className="w-3 h-3" />
          </button>
          <span className="text-[13px] font-semibold text-gray-800">
            {viewMonth.format("MMMM YYYY")}
          </span>
          <button onClick={() => setViewMonth(v => v.add(1, "month"))} className="p-2 rounded-md bg-white border border-gray-200 hover:bg-gray-50 text-gray-600 transition-colors">
            <FaChevronRight className="w-3 h-3" />
          </button>
        </div>
          ...
          {Array.from({ length: totalCells }).map((_, idx) => {
            ...
            return (
              <button
                ...
                onClick={() => setSelected(date.toDate())}
                className={`relative flex flex-col items-center justify-center h-8 w-full rounded-md text-[12px] font-medium transition-all
                  ${isSelected ? "bg-[#6e58d1] text-white" : "text-gray-700 hover:bg-gray-200"}`}
              >
                {dayNum}
      ...
      <div className="px-5 py-5 flex-1">
        <div className="flex items-center justify-between mb-3">
          <p className="text-[10px] font-bold text-gray-500 uppercase tracking-widest">Upcoming</p>
          <span className="bg-gray-200 text-gray-600 border border-gray-300 text-[10px] px-1.5 py-0.5 rounded font-bold">
            {upcomingMeetings.length}
          </span>
        </div>
        <div className="space-y-3">
          {upcomingMeetings.map((e) => (
            <div key={e.meeting_id} className="bg-white border border-gray-200 rounded-lg p-3">
              <div className="flex justify-between items-start mb-1.5">
                <p className="text-[13px] font-semibold text-[#251f47] truncate pr-2">{e.title}</p>
              </div>
              <div className="text-[11px] text-gray-500 flex flex-col gap-1">
                <span className="flex items-center gap-1.5"><FaClock className="w-2.5 h-2.5" /> {fmtTime(e.start_time)} – {fmtTime(e.end_time)}</span>
              </div>
...
export default function MeetingScheduler() {
  ...
  return (
    <div className="flex h-[calc(100vh-120px)] bg-white font-sans overflow-hidden text-gray-800 selection:bg-[#6e58d1]/30">
      ...
      <main className="flex-1 flex flex-col min-w-0 overflow-hidden relative bg-white">
        <header className="px-8 pt-8 pb-4 flex flex-col md:flex-row items-start md:items-center justify-between shrink-0 border-b border-gray-200">
          <div>
            <h1 className="text-[22px] font-semibold text-gray-900 tracking-wide">
          ...
          <div className="flex items-center gap-3 mt-4 md:mt-0">
            {(config?.actionButtons || []).map((btn) => (
              <button
                ...
                className={`flex items-center gap-2 px-5 py-2.5 text-[13px] rounded-lg border border-gray-200 hover:bg-gray-100 text-gray-900 transition-all font-semibold shadow-sm ${btn.className || ''}`}
              >
                <span className="text-gray-700">{btn.icon}</span>
                {btn.label}
              </button>
            ))}
            <button className="p-2.5 rounded-lg border border-gray-200 hover:bg-gray-100 text-gray-600 transition-all shadow-sm">
              <FaEllipsisH />
            </button>
          </div>
        </header>
        ...
        <div className="px-8 pb-6 shrink-0 border-b border-gray-200">
          <div className="inline-flex rounded-xl border border-gray-200 bg-transparent p-[3px]">
            {(config?.tabConfig || []).map(({ key, label }) => (
              <button
                key={key}
                onClick={() => setActiveTab(key)}
        ...
        <div className="flex-1 overflow-y-auto relative pb-20">
          ...
          {activeTab === "timeline" && (
            <div className="relative">
              {!hasTimelineData ? (
                ...
                <div className="flex flex-col w-full">
                  {HOURS.map((h) => {
                    const blocks = slotsByHour[h] || [];
                    return (
                      <div key={h} className="relative flex w-full min-h-[70px] border-b border-gray-200">
                        <div className="w-24 shrink-0 text-right pr-6 pt-3">
              ...
              <div className="fixed bottom-6 right-1/2 translate-x-1/2">
                <button className="w-10 h-10 rounded-full bg-white border border-gray-200 flex items-center justify-center text-gray-600 hover:text-gray-900 hover:bg-gray-50 shadow-[0_0_15px_rgba(0,0,0,0.1)] transition-colors">
                  <FaArrowDown size={14} />
                </button>
              </div>
            </div>
          ...
          {activeTab === "meetings" && (
            <div className="px-8 py-6 grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
              {meetings.length === 0 ? (
                <p className="text-sm text-gray-400 col-span-full py-10 text-center">No meetings scheduled.</p>
              ) : (
          ...
          {activeTab === "availability" && (
            <div className="px-8 py-6 grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
              {availability.length === 0 ? (
                <p className="text-sm text-gray-400 col-span-full py-10 text-center">No availability blocks set.</p>
              ) : (
      ...
      {openModal && (
        ...
        >
          <div
            className="bg-white border border-gray-200 rounded-2xl shadow-2xl w-full max-w-[960px] max-h-[90vh] flex flex-col overflow-hidden"
            onClick={(e) => e.stopPropagation()}
          >
...
//       {/* Reusable Modal Layer for Create and Edit - DARK COLORS ONLY */}
//       {openModal && (
//         <div
...
//         >
//           <div
//             className="bg-[#1a1c29] border border-[#272938] rounded-2xl shadow-2xl w-full max-w-[960px] max-h-[90vh] flex flex-col overflow-hidden"
//             onClick={(e) => e.stopPropagation()}
//           >
//             <div className="flex items-center justify-between px-6 py-4 border-b border-[#272938] shrink-0 bg-[#1a1c29]">
...
//               <button
//                 onClick={closeModal}
//                 className="w-8 h-8 flex items-center justify-center rounded-full hover:bg-[#272938] text-gray-400 hover:text-gray-200 transition-colors"
//               >
//                 <FaTimes size={14} />
//               </button>
//             </div>
