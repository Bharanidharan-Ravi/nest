...
function buildAttendeeOptions(meeting) {
    const internalParticipants = safeParseList(
        meeting?.InternalParticipants
    );

    const clientParticipants = safeParseList(
        meeting?.ClientParticipants
    );

    return [...internalParticipants, ...clientParticipants]
        .filter(
            (participant) =>
...
export default function MeetingCompleteModal({
    ...
    }, [meeting?.meeting_id, meeting?.start_time, attendeeOptions]);
            ...
            actions: ({ formData }) => [
                {
                    ...
                    onClick: ({ submitForm }) => {
                        const attendees = Array.isArray(formData?.Attendee)
                            ? formData.Attendee
                            : [];
