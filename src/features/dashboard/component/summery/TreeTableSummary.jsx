    ...
    <div className="flex items-center justify-center gap-2">
      <div className="flex gap-[2px] border border-gray-200 p-[2px] rounded-[4px] bg-white shadow-sm">
        {[...Array(steps)].map((_, i) => (
          <div
            key={i}
            className={`w-1.5 h-3 rounded-[0.5px] ${i < filled ? "bg-indigo-600" : "bg-gray-100"
              }`}
          />
        ))}
      </div>
...
export function TimesheetSummary() {
  const { data, view } = useList();
  const filteredData = data.filter(item => item.employeeId === null);
  const filteredData2 = data.filter(item => item.ConsumeTime === null);
  const isReportView = view?.toLowerCase() === "report";
  const [expandedGroups, setExpandedGroups] = useState(new Set());
  const [expandedTickets, setExpandedTickets] = useState(new Set());
  // --------------------------
  // Build Tree Data
  ...
  const treeData = useMemo(() => {
    if (!data) return { totalValue: 0, roots: {} };
    const tree = { totalValue: 0, roots: {} };
    ...
    sortedData.forEach((item) => {
      const repoName = item.repoName || "Unassigned";
      const employeeName = item.employeeName || "Unassigned";
      const projectName = item.projectName || "Unassigned";
      const ticketId =
        item.ticketId ||
        item.issueId ||
      ...
      const rootKey = isReportView ? employeeName : repoName;
      const rootLabel = isReportView ? employeeName : repoName;
      const rootType = isReportView ? "employee" : "repo";

      if (!tree.roots[rootKey]) {
        tree.roots[rootKey] = {
          val: 0,
          label: rootLabel,
          type: rootType,
      ...
      if (!tree.roots[rootKey].projects[projectName]) {
        tree.roots[rootKey].projects[projectName] = {
          val: 0,
          tickets: {},
        };
      }

      if (!tree.roots[rootKey].projects[projectName].tickets[ticketId]) {
        tree.roots[rootKey].projects[projectName].tickets[ticketId] = {
          val: 0,
          latest: item,
          history: [],
        };
      ...
      tree.totalValue += hours;
      tree.roots[rootKey].val += hours;
      tree.roots[rootKey].projects[projectName].val += hours;
      tree.roots[rootKey].projects[projectName].tickets[ticketId].val +=
        hours;
      tree.roots[rootKey].projects[projectName].tickets[
        ticketId
      ].history.push(item);
    });
  ...
  //   if (treeData?.roots) {
  //     setExpandedGroups(
  //       new Set(Object.keys(treeData.roots).map((k) => `root_${k}`))
  //     );
  //   }
  // }, [treeData]);
  ...
  const totalRootsCount = Object.keys(treeData.roots).length;
  const isAllExpanded =
    totalRootsCount > 0 && expandedGroups.size >= totalRootsCount;
  const isAllCollapsed = expandedGroups.size === 0;
    ...
    <div className="w-full bg-white rounded-xl border border-gray-200 shadow-sm mt-4 overflow-hidden">
      ...
      <div className="p-4 border-b border-gray-200 flex justify-between items-center bg-gray-50/80">
        <h4 className="font-bold text-gray-700 text-sm uppercase tracking-wider flex items-center gap-2">
          ...
          {isReportView
            ? "Report View – Employee Breakdown"
        ...
        <div className="flex items-center gap-3">
          <div className="flex bg-white rounded-md border border-gray-200 shadow-sm overflow-hidden">
            <button
              onClick={() => toggleAll(true)}
              className={`px-3 py-1.5 text-xs font-semibold border-r border-gray-200 transition-colors flex items-center gap-1 ${isAllExpanded
            ...
            <button
              onClick={() => toggleAll(false)}
              className={`px-3 py-1.5 text-xs font-semibold transition-colors flex items-center gap-1 ${isAllCollapsed
                ? "bg-indigo-50 text-indigo-700"
                : "text-gray-600 hover:bg-gray-50 hover:text-indigo-600"
                }`}
            >
          ...
          <div className="font-bold text-gray-900 bg-white px-4 py-1.5 rounded-md border border-gray-200 shadow-sm flex items-center gap-2 text-sm">
            ...
            {formatHours(treeData.totalValue)}
          </div>
        </div>
      </div>

      {/* Body */}
      {Object.entries(treeData.roots)
        .sort(([, a], [, b]) => {
          if (!isReportView) return 0;

          const nameA = a.label || "undefined";
          const nameB = b.label || "undefined";

          const undefinedA = nameA.toLowerCase() === "undefined";
          const undefinedB = nameB.toLowerCase() === "undefined";

          if (undefinedA && !undefinedB) return 1;
          if (!undefinedA && undefinedB) return -1;
        ...
        }).map(([rootKey, rootData]) => {
          const isOpen = expandedGroups.has(`root_${rootKey}`);
          ...
          return (
            ...
            >
              ...
              <div
                ...
                onClick={() => toggleGroup(`root_${rootKey}`)}
              >
                <div className="flex items-center gap-2 font-bold text-indigo-900 uppercase text-xs tracking-wider">
                  {isOpen ? (
                  ...
                  {rootData.type === "employee" ? (
                    <>
                      <Icon name="User" className="text-indigo-500" size={16} />
                      {rootData.label}
                    </>
                  ) : (
                    <>
                      <Icon name="Folder" className="text-amber-500" size={16} />
                      {rootData.label}
                    </>
                  )}
                </div>
                ...
                <div className="font-bold text-gray-700 text-xs">
                  {formatHours(rootData.val)}
                </div>
              </div>

              {isOpen &&
                Object.entries(rootData.projects).map(([projName, projData]) => (
                  <div
                    key={projName}
                  ...
                  >
                    ...
                    <div className="flex justify-between items-center px-10 py-3 bg-indigo-50/30">
                      <div className="flex items-center gap-2 font-semibold text-indigo-700 text-xs">
                        <Icon name="Briefcase" className="text-blue-500" />
                        {projName}
                      </div>
                    </div>
                    ...
                    <div className="px-10 pb-4">
                      <table className="w-full table-fixed">
                        <thead className="text-[10px] text-gray-400 uppercase tracking-widest border-b border-gray-100">
                          <tr>
                            <th className="text-left py-2 pb-3 w-[40%] font-semibold pl-2">
                              Ticket
                            </th>
                            <th className="text-center py-2 pb-3 w-[15%] font-semibold">
                              Total Time
                            </th>
                            <th className="text-center py-2 pb-3 w-[20%] font-semibold">
                              Progress
                            </th>
                            <th className="text-left py-2 pb-3 w-[25%] font-semibold pl-4">
                              Latest Summary
                            </th>
                          </tr>
                        </thead>
                        ...
                        <tbody>
                          {Object.entries(projData.tickets).map(
                            ([ticketId, ticketGroup]) => {
                              const t = ticketGroup.latest;
                              const hasHistory = ticketGroup.history.length > 0;
                              const isExpanded = expandedTickets.has(ticketId);
                              ...
                              return (
                                <React.Fragment key={ticketId}>
                                  <tr className="group border-b border-gray-50 hover:bg-gray-50/50 transition-colors">
                                    ...
                                    <td
                                      className="py-3 pl-4 text-xs text-gray-500 truncate"
                                      title={t.CurrentStatusSummary || t.Comment}
                                    >
                                      {t.CurrentStatusSummary || t.Comment ? (
