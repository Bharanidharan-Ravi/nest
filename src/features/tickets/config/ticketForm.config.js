...
//         const isClientTicket=formData?.RaiseToClient===true
//         const hasMoveTo=(formData?.move_to?.length ??0)>0

//         console.log("MOVE to check",{
//           RaiseToClient:formData?.RaiseToClient,
//           isClientTicket,
//           move_to:formData?.move_to,
//           hasMoveTo
//         });
        

//         if(isClientTicket && !hasMoveTo){
//           openDialog({
//             variant:"error",
//             title:"Move To Required",
//             description:"This client ticket",
//             confirmText:"OK",
//             onConfirm:()=>{}
//           })
//           return
  ...
  actions: ({ formData, context }) => [
    {
      ...
      onClick: ({ submitForm, context }) => {
        ...
        // 🔥 Client ticket validation: RaiseToClient + move_to
        const isClientTicket = formData?.RaiseToClient === true;
        const hasMoveTo = (formData?.move_to?.length ?? 0) > 0;
        ...
        if (isClientTicket && !hasMoveTo) {
          openDialog({
            ...
            title: "Move To Required",
            description:
              "This is a client ticket. Please select at least one 'Move To' assignee before creating or updating the ticket.",
            confirmText: "OK",
            onConfirm: () => {},
          });
        ...
        openDialog({
          ...
          title: "Some Data is Missing",
          description:
            `To set this ticket to 'Active' or 'Need Confirmation', the following fields are required:\n` +
            `● ${missingFields.join("\n● ")}\n\n` +
            `You can put this ticket in 'In Queue' for now and fill in the details later, or cancel to complete them now.`,
          confirmText: "Queue It (Save)",
          cancelText: "Fill Missing Data",
          onConfirm: () => submitForm({ Status: 18 }), // Force to InQueue
          onCancel: () => {},
        });
      },
    },
