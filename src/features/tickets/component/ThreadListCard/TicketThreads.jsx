...
console.log("threadsData :", threadsData);
  ...
  const threadsArray = Array.isArray(threadsData)
    ? threadsData
    : [];
console.log("rawThreads :", threadsArray);
...
console.log("rawThreads :", rawThreads);
  ...
  const filteredThreads = React.useMemo(() => {
    ...
    if (selectedWorkStream) {
      ...
      assigneesJsonString.forEach((ws) => {
        if (ws.HandOffData) {
          ws.HandOffData.forEach((h) => {
            if (h.TargetStreamId === selectedWorkStream.StreamId) incomingHandoffIds.push(h.HandsOffId);
          });
        }
      });
      ...
      return rawThreads.filter((thread) => {
        ...
        const isByAssignee = thread.CreatedId?.toLowerCase() === targetAssigneeId || thread.CreatedBy?.toLowerCase() === targetAssigneeId;
  ...
  const enrichedTimeline = React.useMemo(() => {
    ...
    filteredThreads.forEach((thread) => {
      combinedTimeline.push({ ...thread, isChatThread: true, sortTime: new Date(thread.createdAt).getTime() });
    });
  ...
  const finalTimeline = React.useMemo(() => {
    ...
    return [
      ...topPart,
      { isCollapsedMarker: true, hiddenCount: remainingHidden, Id: "collapsed-marker" },
      ...bottomPart,
    ];
  ...
  const listConfig = {
    ...
    cardRenderer: (item) => {
      ...
      if (item.isTimelineEvent) {
        ...
        return (
          <div key={item.Id} className="flex items-center gap-4 w-full mb-6 relative">
            ...
            <div className="flex-1 text-[13px] text-gray-600 inline-flex items-center flex-wrap gap-1 w-fit max-w-fit bg-gray-50/50 px-3 py-1.5 rounded-full border border-gray-100">
              {item.summary.startsWith(item.actorName) ? (
                <>
                  <strong className="text-gray-800 mr-1">{item.actorName}</strong>
                  <span>{item.summary.replace(item.actorName, "").trim()}</span>
                </>
              ) : (
                <span>{item.summary}</span>
              )}
              <span className="text-xs text-gray-400 ml-2">• {dayjs(item.createdAt).fromNow()}</span>
            </div>
          </div>
        );
      }
